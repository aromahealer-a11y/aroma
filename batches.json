...
async function main() {
  ...
  // CODES 環境變數可指定「只抓這幾項」（進貨後更新單一商品用），未指定則全部
  const only = (process.env.CODES || '')
    .toUpperCase().split(/[\s,]+/).filter(Boolean);
  const targets = only.length ? only : codes;

  // 這次沒抓的商品，原封不動沿用上次結果
  const products = {};
  for (const code of new Set([...codes, ...only])) {
    if (prev.products?.[code]) products[code] = prev.products[code];
  }
  if (only.length) console.log(`只更新指定商品：${targets.join(' ')}\n`);
  ...
  console.log(`\n完成：本次更新 ${ok} 項、失敗 ${failed} 項，batches.json 共 ${Object.keys(products).length} 項`);
